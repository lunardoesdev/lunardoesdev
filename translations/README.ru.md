...
**static code school proof-of-concept** — Интерактивная браузерная платформа для обучения программированию
Proof-of-concept приложение, включающее редактор кода Monaco и выполнение Go-кода на стороне клиента с использованием скомпилированного в WebAssembly интерпретатора Yaegi. Запуская тесты кода прямо в браузере, оно полностью избавляет от необходимости в дорогих VPS-серверах и бэкенд-исполнителях, позволяя любому желающему бесплатно развернуть полноценную школу программирования на любой статической веб-странице. Приложение парсит уроки и тесты, написанные на Markdown. *(Примечание: при медленном соединении загрузка WebAssembly-интерпретатора может занять некоторое время.)*
...
## Скрипты и Утилиты
**gitbacklist** — Инструмент для инкрементного резервного копирования git-репозиториев с профилей GitHub и git-совместимых хостов с поддержанием локальных зеркал в синхронизированном состоянии.
