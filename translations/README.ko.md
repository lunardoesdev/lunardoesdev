...
**fedorafused** — 맞춤형 Fedora Workstation 라이브 ISO
KIWI NG를 사용한 자동화된 빌드 파이프라인으로 즉시 배포 가능한
맞춤형 Fedora Workstation 라이브 환경을 생성합니다.
...
**singerbench2** — 고성능 프록시 링크 테스터
구독 링크(VLESS, VMess 등)를 입력받아 노드의 가용성을 평가하며,
죽은 엔드포인트를 안정적으로 필터링하여 활성화된 서버만 출력합니다.
...
## 스크립트 및 유틸리티
**gitbacklist** — GitHub 프로필 및 git 호환 호스트를 위한 증분 git 백업 도구로, 로컬 미러를 동기화된 상태로 유지합니다.
...
**ytdla** — 내부적으로 yt-dlp를 사용하여 YouTube에서 오디오를 다운로드하는 독자적인(opinionated) 스크립트.
[GitHub](https://github.com/lunardoesdev/ytdla)

**file2sh** — 파일을 읽고 해당 파일을 재현하는 셸 스크립트를 생성하는 유틸리티.
[GitHub](https://github.com/lunardoesdev/file2sh)

**examplenv** — 시크릿 값을 샘플 값으로 덮어써서 .env에서 안전한 .env.example을 생성하는 유틸리티.
